...
build-installer: manifests generate kustomize ## Generate dist/install.yaml (CRDs+deploy+HelmRepository+ModuleTemplate+GitRepository+addon ModuleTemplates).
	...
	{ "$(KUSTOMIZE)" build config/default; echo "---"; "$(KUSTOMIZE)" build config/addons/helmrepository/module-templates; echo "---"; "$(KUSTOMIZE)" build config/addons/gitrepository/module-templates; } > dist/install.yaml
